...
Notes that the \emph{bookinother} bibliography's style automatically loads \emph{verbose} bibliography's style, which means its compatible with all the standard bibliography's  styles of \biblatex, because all of them are, in a way or an other, based on the \emph{verbose} bibliography style.
 
In any case, you can choose you own citation's style.
 
If you need to use this package with package which also requires loading \emph{via} the \verb+bibstyle+ option, as for example biblatex-morenames, just use the biblatex-multiple-dm package, with the following way:
...
If you use the \bibfield{inbook} field of the \emph{biblatex-morenames} package, it will be inherited as \bibfield{bookeditor} field.
...
\item Adds field \bibfield{bookeditor} and \bibfield{bookineditor}.
\end{release}
