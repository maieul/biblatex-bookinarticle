...
\label{morenames}If you need to use this package with package which also requires loading \emph{via} the \verb+bibstyle+ option, as for example \emph{biblatex-morenames}, just use the \emph{biblatex-multiple-dm} package, in the following way:
...
\docsvlist{%
  ...
  {%
  \unexpanded{\subsubsection{About \bibfield{bookineditor}}
  For technical reason, if you want to use the \bibfield{bookineditor} with the \bibfield{bookinjournal} entry type, you must also use the \emph{biblatex-morenames} package (see \reff{morenames}).
  }}},%
